...
namespace dawn::client::hooks::bootflow::one_au_entrance {
namespace au=state::activity::vanilla::one_au;
namespace entrance=au::entrance_native;
namespace bridge=au::bridge_native;
...
struct World {
    ...
    bool owned(std::uint32_t entity,bool& out) const noexcept {
        std::uint32_t word{};
        if(!copy(reinterpret_cast<const void*>(base+bridge::kAuthorityTable+4*((entity&0x1FFFU)>>5U)),&word,sizeof word)) return false;
        out=(word&(1U<<(entity&31U)))!=0;return true;
    }
    void grant(std::uint32_t entity) const noexcept {
        reinterpret_cast<void(__fastcall*)(std::uint32_t,std::uint8_t) noexcept>(base+bridge::kAuthoritySetter)(entity,1U);
    }
    void retire(std::uint32_t entity) const noexcept {
        reinterpret_cast<void(__fastcall*)(std::uint32_t) noexcept>(base+entrance::kRetire)(entity);
    }
};
// Serialized by busy; report transitions and a bounded heartbeat so a failed
// cleanup is distinguishable from a callback that never ran. No identities or
// native pointers are retained for later mutation.
inline void report(const au::EntranceRequest& wanted,std::string_view status,
            std::uint32_t callbackEntity,const entrance::Result& result={}) noexcept {
    static au::EntranceRequest previous{};
    static std::string_view previousStatus;
    static ULONGLONG next{};
    const auto now=GetTickCount64();
    if(wanted==previous && status==previousStatus && now<next
        && !result.doors && result.retired==UINT32_MAX) return;
    previous=wanted;previousStatus=status;next=now+10000;
    core::log::writef(core::log::Channel::client,core::log::Level::info,
        "ev=one_au stage=entrance_probe status=%.*s run=%llu generation=%u section=%u callback=%08X rows_checked=%u owned_bridges=%u unowned_bridges=%u incomplete=%u callback_self=%u matches=%u doors=%u retired=%08X",
        static_cast<int>(status.size()),status.data(),static_cast<unsigned long long>(wanted.owner.run),
        wanted.generation,static_cast<unsigned>(wanted.section),callbackEntity,result.rows,result.ownedBridges,
        result.unownedBridges,result.incomplete,result.callbackSelf,result.matches,result.doors,result.retired);
}
__declspec(noinline) inline void update(std::uintptr_t image,void* component) noexcept {
    const auto wanted=au::entrance_request();
    ...
    std::uint32_t stride{},callbackEntity{};std::array<unsigned char,16> setter{},retire{};
    ...
    if(!read.value(world.base+bridge::kAuthoritySetter,setter) || setter!=bridge::kSetterPrefix
        || !read.value(world.base+entrance::kRetire,retire) || retire!=entrance::kRetirePrefix) {
    ...
    if(!omega_native_memory::readable(reinterpret_cast<const void*>(world.table),entrance::kRows*entrance::kStride)
        || !omega_native_memory::readable(reinterpret_cast<const void*>(image+bridge::kAuthorityTable),native_authority_bitmap::View::kBytes)) {
        report(wanted,"table_unreadable",callbackEntity);return;
    }
    const auto result=entrance::repair(world,callbackEntity,wanted,[] {return au::entrance_request();});
    report(wanted,"scanned",callbackEntity,result);
    if(result.doors || result.retired!=UINT32_MAX) {
        std::array<char,224> line{};
        const int n=std::snprintf(line.data(),line.size(),"ev=one_au stage=entrance_repair run=%llu generation=%u doors=%u retired=%08X",
            static_cast<unsigned long long>(wanted.owner.run),wanted.generation,result.doors,result.retired);
