...
namespace dawn::server::bap::encrypted::push::activity::homecoming_inventory {
namespace mission=state::activity::vanilla::homecoming;
inline bool consume(Session& session,Scratch& scratch,std::span<std::byte> response,
    ...
    const auto reward=mission::kArmoryRewards[requested.reward];
    ...
    std::snprintf(line.data(),line.size(),"ev=homecoming stage=weapon_grant reward=%u action=%s connection=%u bytes=%zu",
        requested.reward,owned?"equipped":"acquired",session.id,size);
