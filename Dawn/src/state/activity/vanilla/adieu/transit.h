...
inline native::Authority project(ActivityInstanceKey activity,std::uint64_t run,std::uint64_t member,
    ...
    const auto wanted=adieu::request();native::Authority result{};Scope scope{};
    {
        ...
        if(scope!=lastScope || result.host!=lastHost || observation.local!=lastClient
            ...
            std::snprintf(line.data(),line.size(),"ev=adieu stage=transit run=%llu owner=%u wanted=%u route=%u host=%d/%u/%d client=%d/%u/%d region=%d receipt=%u complete=%u",
    ...
    if(result.arrived) {adieu::observe_arrival(scope.owner,scope.route);}
    return result;
}
inline bool membership_due(ActivityInstanceKey activity,std::uint64_t run,std::uint64_t now) noexcept {
    const auto wanted=adieu::request();const std::lock_guard lock(mutex);
