...
namespace dawn::state::activity::vanilla::homecoming::cinematics {
using homecoming::kMovies;
// Package 80B500BC bubble/state-1 owners: intro 80B5012D (bubble 2, region 17), Amanda's
// pickup 80B508FC (bubble 8, region 65) and the outro 80B50126 (bubble 1, region 9).
inline constexpr std::uint8_t kIntro=0,kPickup=1,kOutro=2;
inline constexpr std::uint8_t kUnderwatchRegion=72,kShipRegion=64;
...
struct State {
    ...
    // Routes: 1..3 select a movie region; 4 returns to the Underwatch after the intro;
    // 5 returns to the command ship after Amanda's pickup.
    std::uint8_t route() const noexcept {
        if(phase==Phase::landing) {return movie==kIntro?4:5;}
    ...
    bool ending() const noexcept {return movie==kOutro && phase!=Phase::dormant && phase!=Phase::gameplay;}
};
...
public:
    ...
    // Amanda's pickup leaves the boulevard for region 65 and returns to the ship.
    bool begin_pickup(coo::Generation owner,std::uint64_t now) noexcept {
        if(owner!=state_.owner || state_.phase!=Phase::gameplay || state_.movie!=kIntro) {return false;}prepare(kPickup,now);return true;
    }
    bool finish_gameplay(coo::Generation owner,std::uint64_t now) noexcept {
        if(owner!=state_.owner || state_.phase!=Phase::gameplay || state_.movie!=kPickup) {return false;}prepare(kOutro,now);return true;
    }
    bool arrival(coo::Generation owner,std::uint8_t route,std::uint64_t now) noexcept {
        ...
        if(state_.phase==Phase::landing && (route==4 || route==5)) {state_.phase=Phase::gameplay;state_.deadline=0;return true;}
    ...
    bool incident(coo::Generation owner,std::uint32_t target,std::uint32_t registry,std::int8_t type,
        ...
        if(state_.movie==kIntro || state_.movie==kPickup) {state_.phase=Phase::landing;state_.deadline=now+90000;}
