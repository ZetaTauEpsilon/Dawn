...
inline constexpr coo::script::Profile kEntryProfile{
    "homecoming.native.v1", "otherMissions", coo::Schema::otherMissions,
...
inline bool valid_document(const coo::script::Views& views) noexcept {
    ...
    if (!views.valid || views.missionId != "homecoming" || views.profileId != kEntryProfile.id
...
// A small composition adapter; the native controller owns every mission fact and
// cannot reset the process-owned Lua document or its mission completion wait.
class Entry final {
public:
