...
// Retail timing reference: walkthrough captions (gameplay begins at 03:30). Offsets below
// are seconds after the Underwatch landing: Ghost 1 at +7, the breach and cue 6 at +15/17,
// Zavala's announcement at +24, Cayde at +33, Shaxx at +53, armory music at +68, the hangar
// window at +128, the hangar reveal music and the Red Legion conversation at +159/162, the
// plaza defence from +245, "we hold here" at +319, the barrage at +329, the last wave at
// +360, the shuttles at +365, Ikora at +399, Zavala's pickup order at +409, Holliday
// inbound at +437, her arrival at +455, the deck music at +502, the console at +532/543,
// Cayde's status at +565, the generator room at +666/670, the turbines from +696, the
// shields down at +726/731, the outro at +754.
Mission::Mission() noexcept {
    ...
    // player looks around; only the Cabal arrive through their authored breaches.
    auto& underwatch=phases[static_cast<std::size_t>(Section::underwatch)];underwatch.name("Homecoming / Underwatch");
    auto arrival=underwatch.add("crash site",0,{objective(0),music(ms::underwatchRuins),spawn(Cohort::underwatchCast),device(kUnderwatch,108,0.F),
    ...
    auto b=underwatch.add("first contact scene",a,{music(ms::firstCabal),scene(kUnderwatch,10),spawn(Cohort::breachBackup),speech(4),speech(5,1500),speech(6,4000)});
    ...
    underwatch.add("evacuation line",a,{scene(kUnderwatch,56),scene(kUnderwatch,57),scene(kUnderwatch,58),scene(kUnderwatch,59),scene(kUnderwatch,60),scene(kUnderwatch,61),speech(7)});
    ...
    // The after-Cayde score follows his handoff (the authored volume sits on the same path).
    auto cayde=underwatch.add("collision handoff",c,{object(kUnderwatch,81,false),music(ms::afterCayde)});
    ...
    // Armory: the authored door volume starts the armory score (+68); the post-gun guards
    ...
    armory.add("post gun",a,{scene(kUnderwatch,111),speech(30)});
    ...
    arrival=military.add("hangar route",0,{objective(4),device(kMilitary,40,0.F),device(kMilitary,41,0.F),spawn(Cohort::overlookStart),spawn(Cohort::friendlies),spawn(Cohort::corridor)});
    military.add("fake fight",0,{scene(kMilitary,70),scene(kMilitary,72),scene(kMilitary,74),scene(kMilitary,77)});
    ...
    a=military.add("hallway destruction",c,{music(ms::firstPod),spawn(Cohort::hallwayDestruction)});
    b=military.add("first gate cleared",a,{cleared(Cohort::gateStartClear)});
    c=military.add("first gate opens",b,{device(kMilitary,40,1.F),spawn(Cohort::hangarFloor)});
    ...
    // The reveal volume starts the hangar score; the Red Legion conversation follows it (+162).
    a=military.add("hangar reveal",arrival,{area(kMilitaryRoute,"tv_music_cabal_ship_reveal")});
    military.add("red legion",a,{music(ms::cabalShipReveal),speech(37,3000)});
    ...
    military.add("to the plaza",a,{speech(38),speech(39),finish()});

    // Plaza: Zavala's authored scenes own his actor. The defence follows the captions: the
    // first assault (52, then 51 twenty seconds later), a five-second hold, "we hold here"
    // (55), the barrage ten seconds later with the Ward (57), the strike, the second wave while
    // Zavala is down, his revival, the last wave (54) and the shuttles (59).
    ...
    plaza.add("skybox ships A",arrival,{object(kPlazaProps,0),object(kPlazaProps,5),object(kPlazaProps,4),object(kPlazaProps,9),object(kPlazaProps,10),object(kPlazaProps,11),object(kPlazaProps,12)});
    ...
    auto first=plaza.add("first assault",b,{speech(52),speech(51,20000),object(kPlaza,45),spawn(Cohort::wave1),spawn(Cohort::interim)});
    ...
    b=plaza.add("we hold here",a,{speech(55,5000)});
    b=plaza.add("barrage warning",b,{after(15000)});
    c=plaza.add("barrage",b,{speech(57),mechanic(asset(kPlaza,43,3),Mechanic::stopScene),scene(kPlaza,4),object(kPlaza,44,false),object(kPlaza,46,false),object(kPlaza,47,false),mechanic(asset(kPlaza,23,48),Mechanic::powerOff)});
    ...
    a=plaza.add("barrage strike",c,{object(kPlaza,18,false),object(kPlaza,44),mechanic(asset(kPlaza,43,4),Mechanic::stopScene),scene(kPlaza,5),spawn(Cohort::wave2)});
    ...
    plaza.add("last assault approaches",c,{finish()});
    ...
    a=waves.add("last assault intermission",arrival,{after(3000)});
    b=waves.add("last assault",a,{speech(54),spawn(Cohort::wave3),object(kPlazaProps,18),object(kPlazaProps,22),object(kPlazaProps,23),object(kPlazaProps,24)});
    ...
    a=waves.add("second wave cleared",b|c,{cleared(Cohort::wave2)});
    b=waves.add("last assault cleared",a,{cleared(Cohort::wave3)});
    ...
    waves.add("boulevard",b,{music(ms::leavingPlaza),speech(60),speech(61),finish()});
    ...
    a=boulevard.add("Speaker report",blast,{after(10000)});
    ...
    auto placed=boulevard.add("bazaar placed",a,{spawn(Cohort::bazaarStart),spawn(Cohort::bazaarMid),spawn(Cohort::bazaarFar),spawn(Cohort::bazaarEnd),speech(74)});
    ...
    a=boulevard.add("Ikora's Cabal blasted",blast,{after(8000)});
    ...
    // Command ship: the deck score starts on landing (+502), the console room is populated
    // before the scan, the hologram line precedes the scan (+532) and the route lines follow
    // it (+543, +565). Console doors follow the matching scan generation.
    auto& ship=phases[static_cast<std::size_t>(Section::ship)];ship.name("Homecoming / Command ship");
    arrival=ship.add("ship arrival",0,{objective(13),music(ms::battleshipDeck),speech(77,3000),speech(78,25000),spawn(Cohort::pods),ghost(kShip,166,true)});
    ship.add("pod bays",0,{spawn(Cohort::damaged),spawn(Cohort::hall),spawn(Cohort::stairs)});
    a=ship.add("console scanned",arrival,{reached(Milestone::consoleScanned)});
    ship.add("console doors",a,{ghost(kShip,166,false),device(kShip,167,1.F),device(kShip,56,1.F),device(kShip,57,1.F),objective(14),speech(79),speech(82,20000)});
    ...
    ship.add("stair impacts",a,{object(kShip,102),object(kShip,104),object(kShip,106),object(kShip,108),object(kShip,110),spawn(Cohort::deckA)});
    ...
    ship.add("stairs door",a,{device(kShip,58,1.F),spawn(Cohort::deckB),spawn(Cohort::hardpoints)});
    ...
    // sabotage score starts in the generator chamber (+670) and the turbine lines follow
    // the destruction receipts (+699 first, +715 second).
    auto& generator=phases[static_cast<std::size_t>(Section::generator)];generator.name("Homecoming / Shield generator");
    arrival=generator.add("boss announce",0,{spawn(Cohort::boss),sequence(kShip,174),spawn(Cohort::airlock),spawn(Cohort::matrix),spawn(Cohort::engineUpper),spawn(Cohort::engineLower)});
    ...
    b=generator.add("generator entrance opens",a|approach,{device(kShip,59,1.F),speech(85)});
    ...
    generator.add("overload",chamber,{music(ms::battleshipSabotage),objective(11),speech(86),spawn(Cohort::generatorGuards),object(kShip,142),object(kShip,144),object(kShip,146),object(kShip,148)});
    auto turbines=generator.add("turbines",chamber,{object(kShip,149),device(kShip,143,.1F),device(kShip,145,.1F),device(kShip,147,.1F),device(kShip,61,.1F),device(kShip,62,.1F),device(kShip,64,.1F),device(kShip,63,1.F)});
    generator.add("turbine lights",chamber,{device(kShip,159,1.F),device(kShip,160,1.F),device(kShip,161,1.F),device(kShip,162,1.F),device(kShip,65,1.F),device(kShip,66,1.F),device(kShip,67,1.F)});
    a=generator.add("turbine A destroyed",turbines,{reached(Milestone::generatorA)});
    auto killA=generator.add("turbine A dark",a,{device(kShip,160,0.F),speech(88)});
    a=generator.add("turbine B destroyed",turbines,{reached(Milestone::generatorB)});
    auto killB=generator.add("turbine B dark",a,{device(kShip,161,0.F),speech(90)});
    ...
    b=generator.add("escape opens",a,{music(ms::run),object(kShip,76),device(kShip,163,1.F),device(kShip,60,1.F),objective(12),speech(91),speech(92,5000),spawn(Cohort::escape)});
    ...
    const auto valid=[](const Graph& graph) {
        ...
        for(const auto& step:graph.definition.steps) for(const auto& command:step.commands) {
            ...
            if(command.operation==coo::Operation::population) {
                // Loose cohorts never contain Scene-owned sources; those spawn through their Scene.
                if(command.asset!=kModule || command.argument>=std::size(kCohorts)) {return false;}
                for(const auto member:kCohorts[command.argument].members) {
                    bool loose=false;
