...
// Homecoming's launch profile leaves the spawn set unset so the authored arrival owns the
// point inside each slice. The canonical absent hash requests that same authored arrival.
inline constexpr std::uint32_t kAuthoredArrival=0x811C9DC5U;
constexpr native::Target destination(std::uint8_t route) noexcept {
    return route>=1 && route<=3?native::Target{cinematics::kMovies[route-1].region,kAuthoredArrival}
        :route==4?native::Target{cinematics::kUnderwatchRegion,kAuthoredArrival}
        :route==5?native::Target{cinematics::kShipRegion,kAuthoredArrival}:native::Target{};
}
// Gameplay boundaries stream through the native current/pending region legs. Only
// the intro, the returns from a movie, Amanda's pickup and the outro request C9.
