...
inline native::Authority project(ActivityInstanceKey activity,std::uint64_t run,std::uint64_t member,
    ...
    const auto wanted=one_au::request();native::Authority result{};Scope scope{};
    {
        ...
        if(scope!=lastScope || result.host!=lastHost || observation.local!=lastClient
            ...
            std::snprintf(line.data(),line.size(),"ev=one_au stage=transit run=%llu owner=%u wanted=%u route=%u host=%d/%u/%d client=%d/%u/%d region=%d receipt=%u complete=%u",
    ...
    if(result.arrived) {one_au::observe_arrival(scope.owner,scope.route);}
    return result;
}
inline bool membership_due(ActivityInstanceKey activity,std::uint64_t run,std::uint64_t now) noexcept {
    const auto wanted=one_au::request();const std::lock_guard lock(mutex);
    ...
    const bool recovery=wanted.frame.recovery.holding();
    ...
    if((!pending && !requested && !recovery) || now<nextRefresh) {return false;}nextRefresh=now+100;return true;
}
}
